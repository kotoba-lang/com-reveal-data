(ns reveal_data.main
  "Kotodama WASM entrypoint for the reveal_data clean-room actor (L4) — Clojure port.
  ...
  facts (`reveal_data.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "reveal_data")
(def tier "L4")
...
(def entity-specs
  [{:entity "Matter"   :plural "matters"   :id-prefix "revealda_mat"
    :fields [:clientId :name :status :openedAt]      :required [:name :status]
    :coerce {}                       :refs {}}
   {:entity "Document"      :plural "documents"      :id-prefix "revealda_doc"
...
(defn emit-facts
  "EAVT facts for one record: {\"reveal_data.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "reveal_data-compat" :tier tier :entities entities} 200])
...
(defn start! [] :reveal_data-compat/ready)
